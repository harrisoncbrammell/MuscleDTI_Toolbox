function [smoothed_fiber_all, fiber_all_mm, smoothed_fiber_all_mm, pcoeff_r, pcoeff_c, pcoeff_s, n_points_smoothed, residuals, residuals_mm] = ...
...
%
%USAGE
%  The function fiber_smoother, v. 1.4.0, is used to smooth fiber tracts and increase
...
%            smoothing), 16 Feb 2024, Roberto Pineda Guzman
%  v. 1.4.0  (ensured compatibility with new fiber_all matrix sizes to
%            correspond to new seeding options), 5 August 2025, Bruce Damon
%
%ACKNOWLEDGEMENTS
...
max_length = size(fiber_all, 3); %#ok<MXFND>
...
if strcmp(fs_options.tract_units, 'vx') || strcmp(fs_options.tract_units, 'VX')
    fiber_all_mm(:,:,:,1) = fiber_all(:,:,:,1)*(dwi_fov/dwi_xsize);                                                %convert fiber tracts in pixels to mm
    fiber_all_mm(:,:,:,2) = fiber_all(:,:,:,2)*(dwi_fov/dwi_xsize);         %previous version squeezed the                                   matrix. Changed this to allow voxel seeding
...
for row_cntr = 1:length(fiber_all_mm(:,1,1,1))
    for col_cntr = 1:length(fiber_all_mm(1,:,1,1))
        ...
        loop_fiber_idx = find(fiber_all_mm(row_cntr,col_cntr,:,1));
        loop_fiber_length_points = length(loop_fiber_idx);
        
        if loop_fiber_length_points > (2*(max(p_order)))                                                                    %only keep and smooth tracts that have (2 * the largest polynomial order) points
            
            fiber_distance = [];
            fiber_distance(:,1) = nonzeros(squeeze(fiber_all_mm(row_cntr,col_cntr,:, 1)));                        %row, column, and slice positions
            fiber_distance(:,2) = nonzeros(squeeze(fiber_all_mm(row_cntr,col_cntr,:, 2)));                        %row, column, and slice positions
            fiber_distance(:,3) = nonzeros(squeeze(fiber_all_mm(row_cntr,col_cntr,:, 3)));                        %row, column, and slice positions
            ...
            loop_fiber_r = squeeze(fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx, 1));                           	%get raw tract data in row direction
            ...
            loop_fiber_c = squeeze(fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx, 2));                          %get raw tract data in column direction
            ...
            loop_fiber_s = squeeze(fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx, 3));                          %get raw tract data in z direction
            ...
            if interpolation_step == 1
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,1) = squeeze(smoothed_fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,1)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,1));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,2) = squeeze(smoothed_fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,2)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,2));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,3) = squeeze(smoothed_fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,3)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,3));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,4) = loop_fiber_idx;                  %number of points
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,5) = 100*(loop_fiber_idx)./loop_fiber_length_points; %percent length
            ...
            % If smoothed fibers have been interpolated to a different step size, evaluate at original step size of input non-smoothed fiber tracts to allow point-to-point residual calculation
                ...
                loop_fitted_fiber_s = loop_fitted_fiber_s - loop_fitted_fiber_s(1);                                          %subtract new fitting offset from all points 
                loop_fitted_fiber_s = loop_fitted_fiber_s + slc_init;                                                        %add back the initial value
                smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,1:length(loop_fitted_fiber_s), 3) = loop_fitted_fiber_s;

                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,1) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,1)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,1));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,2) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,2)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,2));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,3) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,3)-fiber_all_mm(row_cntr,col_cntr,loop_fiber_idx,3));
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,4) = loop_fiber_idx;                   %number of points
                residuals_mm(row_cntr,col_cntr,loop_fiber_idx,5) = 100*(loop_fiber_idx)./loop_fiber_length_points; %percent length
            end
        end
...
smoothed_fiber_all(:,:,:,1) = smoothed_fiber_all_mm(:,:,:,1)/(dwi_fov/dwi_xsize);                                   %convert fiber tracts in mm to pixels
smoothed_fiber_all(:,:,:,2) = smoothed_fiber_all_mm(:,:,:,2)/(dwi_fov/dwi_xsize);       %previous version squeezed the _mm matrix. Changed this to allow voxel seeding
smoothed_fiber_all(:,:,:,3) = smoothed_fiber_all_mm(:,:,:,3)/dwi_slicethickness;
...
for row_cntr = 1:length(fiber_all_mm(:,1,1,1))
    for col_cntr = 1:length(fiber_all_mm(1,:,1,1))
        if interpolation_step == 1
            ...
            loop_fiber_length_points = length(find(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,:,1)));
        end
        if loop_fiber_length_points > (2*(max(p_order))) 
            if interpolation_step == 1
                residuals(row_cntr,col_cntr,loop_fiber_idx,1) = squeeze(smoothed_fiber_all(row_cntr,col_cntr,loop_fiber_idx,1)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,1));
                residuals(row_cntr,col_cntr,loop_fiber_idx,2) = squeeze(smoothed_fiber_all(row_cntr,col_cntr,loop_fiber_idx,2)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,2));
                residuals(row_cntr,col_cntr,loop_fiber_idx,3) = squeeze(smoothed_fiber_all(row_cntr,col_cntr,loop_fiber_idx,3)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,3));
                residuals(row_cntr,col_cntr,loop_fiber_idx,4) = loop_fiber_idx;                      %number of points
                residuals(row_cntr,col_cntr,loop_fiber_idx,5) = 100*(loop_fiber_idx)./loop_fiber_length_points; %percent length
            else
                residuals(row_cntr,col_cntr,loop_fiber_idx,1) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,1)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,1));
                residuals(row_cntr,col_cntr,loop_fiber_idx,2) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,2)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,2));
                residuals(row_cntr,col_cntr,loop_fiber_idx,3) = squeeze(smoothed_fiber_all_mm_orig_inc(row_cntr,col_cntr,loop_fiber_idx,3)-fiber_all(row_cntr,col_cntr,loop_fiber_idx,3));
                residuals(row_cntr,col_cntr,loop_fiber_idx,4) = loop_fiber_idx;                      %number of points
                residuals(row_cntr,col_cntr,loop_fiber_idx,5) = 100*(loop_fiber_idx)./loop_fiber_length_points; %percent length
            end
        end
