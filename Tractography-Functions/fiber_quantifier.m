...
%  architectural parameters pennation angle, fiber tract length, and curvature 
...
%           added calculation of additional orientation angles), 5 Aug
%           2025, Bruce Damon
%
% ACKNOWLEDGEMENTS
    ...
    case{'a'}
        ...
        for row_cntr = start_row:end_row
            ...
            for col_cntr = start_col:end_col
                ...
                centroid_idx = min([length(centroids_diff_all) find(centroids_fitted_all(:,max_distance_idx)>=seed_point_mm(max_distance_idx), 1)]);
    ...
    otherwise % voxel or planar seeding methods
        ...
        for seed_cntr = 1:length(fiber_all_mm(:,1,1,1))
            ...
            for plane_cntr = 1:length(fiber_all_mm(1,:,1,1))
                ...
                % find centroid's direction vector at the seed point
                centroid_idx = min([length(centroids_diff_all(:,1)) find(centroids_fitted_all(:,max_distance_idx)>=seed_point_mm(max_distance_idx), 1)]);
                c_vector = centroids_diff_all(centroid_idx,:);
