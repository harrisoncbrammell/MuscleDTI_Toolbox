...
%
%USAGE
%  The function fiber_visualizer, v. 1.2, is used to visualize images and the 
%  muscle mask, roi mesh, and/or fiber tracts formed using the MuscleDTI_Toolbox.
%
...
%   -anat_alpha: (Optional) If set to 0, the anatomical images will be transparent.
%      If set to 1, the anatomical images will be fully visible.  Intermediate
%      values will adjust the transparency accordingly. The default setting
%      is 1.
...
%  v. 1.1.0 (Added option to change alpha level of the anatomical images),
%     7 Jun 2024, Bruce Damon. 
%  v. 1.2.0 Correct geometric scaling of fibers during display with 
%     anatomical images), 5 Aug 2025, Bruce Damon. 
%
%ACKNOWLEDGEMENTS
...
if isfield(fv_options, 'anat_alpha')
    anat_alpha = fv_options.anat_alpha;
else 
    anat_alpha = 1;
end
...
if fv_options.plot_fibers==1                                                  %convert from pixels to desired units, as input by users
    fiber_all(:,:,:,1) = fiber_all(:,:,:,1)*dti_inplane_res;%dti_inplane_ratio;
    fiber_all(:,:,:,2) = fiber_all(:,:,:,2)*dti_inplane_res;%dti_inplane_ratio;
...
for s=1:length(anat_slices)
    ...
    loop_image = imresize(loop_image, round(anat_inplane_res*[anat_size(1) anat_size(2)]));
    loop_image = 240*loop_image/max(max(loop_image));
    surf_image = surf(ones(size(loop_image, 1), size(loop_image, 2))*double(anat_slicethick)*double(anat_slices(s)), loop_image);
    set(surf_image, 'facealpha', anat_alpha)
    colormap('gray')
    shading('Interp')
end
...
if fv_options.plot_mesh==1
    if numel(mesh_color)==3
        mesh_plot=surf((roi_mesh(:, :, 2) + mesh_dist*roi_mesh(:, :, 5))*mesh_inplane_res, ...
            (roi_mesh(:, :, 1) + mesh_dist*roi_mesh(:, :, 4))*mesh_inplane_res, ...
...
if fv_options.plot_fibers==1
    ...
    if numel(fiber_color)==3
        ...
        for row_cntr = 1:fiber_skip:(length(fiber_all(:,1,1,1)))
            
            for col_cntr = 1:fiber_skip:(length(fiber_all(1,:,1,1)))
                num_points = length(find(fiber_all(row_cntr,col_cntr,:,1)));
                if num_points>5
                    
                    fiber_plot=plot3(nonzeros(squeeze(fiber_all(row_cntr,col_cntr, :, 2))), ...
                        nonzeros(squeeze(fiber_all(row_cntr,col_cntr, :, 1))), ...
                        nonzeros(squeeze(fiber_all(row_cntr,col_cntr, :, 3))));
    ...
    elseif numel(size(fiber_color))==3
        ...
        for row_cntr = 2:fiber_skip:(length(fiber_all(:,1,1,1))-1)
            
            for col_cntr = 2:fiber_skip:(length(fiber_all(1,:,1,1))-1)
        ...
        if numel(fiber_color)==6
            ...
            for row_cntr = 1:fiber_skip:(length(roi_mesh(:,1,1)))
                ...
                for col_cntr = 1:fiber_skip:(length(roi_mesh(1,:,1)))
                    ...
                    fiber_plot=plot3(nonzeros(squeeze(fiber_all_f(row_cntr,col_cntr, :, 2))), ...
                        nonzeros(squeeze(fiber_all_f(row_cntr,col_cntr, :, 1))), ...
                        nonzeros(squeeze(fiber_all_f(row_cntr,col_cntr, :, 3))));
