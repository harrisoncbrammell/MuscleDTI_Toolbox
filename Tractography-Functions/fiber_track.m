...
switch seed_method
    ...
    case{'r'} %seed in image rows
        ...
        seed_points = zeros(2, length(planes), 3);                          %initialize roi_mesh for the planar case, pad with extra zeros in dim. 3
    ...
    case{'v'} %seed in voxels distributed throughout the muscle volume
        ...
        if skip_vxl==1  %every voxel in the eroded slice ROIs

            mask_seed = diffusion_mask;

        elseif skip_vxl>1 %every nth voxel in the eroded slice ROIs
...
if seed_method(1)=='a'
    ...
    for row_cntr=1:length(roi_mesh(:, 1, 1))                                  %start of the row loop
        ...
        for col_cntr=1:length(roi_mesh(1, :, 1))                              %start of the column loop
            ...
            if diffusion_mask(e1fa_r,e1fa_c,e1fa_s)==0                       
            ...
            while 1
                ...
                if step_dir(1)>0
                    e1fa_r = floor(next_point(1));
                elseif step_dir(1)<0
                    e1fa_r = ceil(next_point(1));
                else
                    e1fa_r = round(next_point(1));
                end
                
                if step_dir(2)>0
                    e1fa_c = ceil(next_point(2));
                elseif step_dir(2)<0
                    e1fa_c = floor(next_point(2));
                else
                    e1fa_c = round(next_point(2));
                end %round the coordiantes to get a row, column, and slice location in the tensor matrix

                e1fa_s = max([round(next_point(3)) 1]);                           %just in case the z coordinate rounds to zero, make it a one

                % terminate tract if out of bounds and record as stop criterion = 4
                if diffusion_mask(e1fa_r,e1fa_c,e1fa_s)==0  
    ...
    for plane_cntr=1:length(seed_points(1,:,1))
        ...
        for seed_cntr=1:length(seed_points(:,plane_cntr,1))  %start of the seed point loop
            ...
            while 1
                ...
                if diffusion_mask(e1fa_r_2,e1fa_c_2,e1fa_s_2)==0 || next_point_2(3)<0.5
                    stop_list_2(seed_cntr, plane_cntr) = 4;
                    break;
                end
...
if seed_method(1) ~='a'
    ...
    if sum(sum(sum(internal_mask)))>0 %if there is an internal mask
...
save temp_fiber_tract_data fiber_all roi_flag stop_list fiber_len
