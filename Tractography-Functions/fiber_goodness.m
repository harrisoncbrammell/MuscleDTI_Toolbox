function [selected_fibers, selected_curvature, selected_angle, selected_distance, qual_mask, num_tracked, mean_fiber_props, mean_apo_props] = ...
...
%  [selected_fibers, final_curvature, final_angle, final_distance, qual_mask, num_tracked, mean_fiber_props, mean_apo_props] = ...
%     fiber_goodness(smoothed_fiber_all, angle_list, distance_list, curvature_list, n_points, roi_flag, apo_area, fg_options);
%
%USAGE
%  The function fiber_goodness, v. 1.1.2, is used to assess the goodness of fiber   
%  tract data, and reject outlying fiber tract data, in the MuscleDTI_Toolbox. 
%  
...
%
%OUTPUT ARGUMENTS
%  selected_fibers: the fiber tracts that passed all selection criteria
%
...
%  v. 1.1.0 (bug fix - corrected reporting of num_tracked fibers), 11 Aug 2021, 
%     Bruce Damon
%  v. 1.1.1 (renamed final_XX variations to selected_XX), 30 Oct 2021, Bruce Damon
%  v. 1.1.2 (updated help), 5 Aug 2025, Bruce Damon
%
%ACKNOWLEDGEMENTS
...
selected_fibers=zeros(size(fiber_all));                                        %initialize matrix of fiber fibers; 
selected_fibers(first_row:last_row, first_col:last_col, :, :) = ...            %set as tracked fibers; wil prune erroneus results later
    fiber_all(first_row:last_row, first_col:last_col, :, :);
selected_angle = angle_list;                                                   %same for geometric measurements of tracts
selected_distance = distance_list;
selected_curvature = curvature_list;
...
for row_cntr = (first_row+2):(last_row-2)
    ...
    for col_cntr = (first_col+2):(last_col-2)
        ...
        selected_fibers(row_cntr,col_cntr,:,1) = selected_fibers(row_cntr,col_cntr,:,1)*qual_mask(row_cntr,col_cntr,6);
        selected_fibers(row_cntr,col_cntr,:,2) = selected_fibers(row_cntr,col_cntr,:,2)*qual_mask(row_cntr,col_cntr,6);
        selected_fibers(row_cntr,col_cntr,:,3) = selected_fibers(row_cntr,col_cntr,:,3)*qual_mask(row_cntr,col_cntr,6);
        
        %eliminate architecture measurements for failed tracts
        selected_curvature(row_cntr,col_cntr,:) = selected_curvature(row_cntr,col_cntr,:)*qual_mask(row_cntr,col_cntr,6);
        selected_angle(row_cntr,col_cntr,:) = selected_angle(row_cntr,col_cntr,:)*qual_mask(row_cntr,col_cntr,6);
        selected_distance(row_cntr,col_cntr,:) = selected_distance(row_cntr,col_cntr,:)*qual_mask(row_cntr,col_cntr,6);
